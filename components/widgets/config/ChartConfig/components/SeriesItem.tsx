...
export const SeriesItem: React.FC<SeriesItemProps> = ({
  ...
  const [titleValue, setTitleValue] = useState(
    series.displayName || getDisplayTitle(series, variables)
  );
  ...
  const displayTitle = getDisplayTitle(series, variables);
  ...
  const getIndicatorLabel = (): React.ReactNode => {
    ...
    const isDynamic =
      ...
      series.indicator !== null &&
      "type" in series.indicator &&
      series.indicator.type === "variable";

    if (isDynamic) {
      const variableBinding = series.indicator as {
        type: "variable";
        key: string;
      };
    ...
    const text =
      typeof series.indicator === "string"
        ? series.indicator
        : String(series.indicator);
  ...
  useEffect(() => {
    if (!series.displayName && !isEditing) {
      setTitleValue(getDisplayTitle(series, variables));
    }
  }, [variables, series, isEditing]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      if (titleValue.trim() !== "") {
        ...
        setTitleValue(series.displayName || getDisplayTitle(series, variables));
      }
      setIsEditing(false);
  ...
  return (
    ...
    >
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        if (titleValue.trim() !== "") {
                          ...
                          setTitleValue(
                            series.displayName ||
                              getDisplayTitle(series, variables)
                          );
                        }
                        setIsEditing(false);
                      ...
                      data-tooltip-id={`series-action-tooltip-${id}`}
        ...
        {!isEditing && (
          <>
            ...
            <button
              ...
              data-tooltip-id={`series-action-tooltip-${id}`}
              data-tooltip-content={
                isVisible ? "Ocultar serie" : "Mostrar serie"
              }
            >
            ...
            <button
              ...
              data-tooltip-id={`series-action-tooltip-${id}`}
              data-tooltip-content="Copiar serie"
            >
            ...
            <button
              ...
              className="column-item__action"
              data-tooltip-id={`series-action-tooltip-${id}`}
              data-tooltip-content="Editar serie"
            >
            ...
            <button
              ...
              data-tooltip-id={`series-action-tooltip-${id}`}
              data-tooltip-content="Eliminar serie"
            >
          ...
          <div className="column-item__chips-content">
            <div className="metric-selector__chip-modifiers">
              ...
              <span
                ...
                key={`${series.id}-indicator`}
              >
                {getIndicatorLabel()}
              </span>

              {/* Chips de modificadores */}
              {Object.entries(series.modifiers).map(([modKey, modValue]) =>
                modValue ? (
                  <span
      ...
      {/* Tooltip único para esta serie */}
      <Tooltip id={`series-action-tooltip-${id}`} place="top" />
    </div>
  );
};
